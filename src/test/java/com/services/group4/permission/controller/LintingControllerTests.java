...
public class LintingControllerTests {
  ...
  void testGetConfig_UserFound() throws Exception {
    ...
    mockMvc.perform(get("/linting/rules")
...
//    when(lintingService.asyncLint(snippetsIds, rulesDto)).thenReturn(Optional.of(snippetsIds.size()));
//
...
//  }
  ...
  void testUpdateRulesAndLint_ServerError() throws Exception {
    ...
    when(ownershipService.findSnippetIdsByUserId(userId)).thenThrow(new RuntimeException("Internal server error"));
